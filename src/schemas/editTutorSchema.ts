...
const editTutorSchema = z.object({
  ...
  email: z
    .optional(z.string().trim().email("Email precisa ser válido"))
    .or(z.literal("")),
  endereco: z
    .string()
    .trim()
  ...
  profissao: z
    ...
    .max(50, "Profissão precisa ter no mínimo 50 caracteres"),
  numCriancas: z
    .number()
  ...
  status: z
    .string()
    .refine(
      (value) => ["aprovado", "reprovado", "em análise"].includes(value),
      "Selecione uma opção válida"
    ),
});

export default editTutorSchema;
