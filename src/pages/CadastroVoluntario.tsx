...
const schema = z.object({
  ...
  senha: z.string().trim().min(6, 'Senha precisa ter no mínimo 6 caracteres').max(30, 'Senha precisa ter no máximo 30 caracteres'),
})
...
export default function CadastroVoluntario() {
  ...
  const { register, handleSubmit, formState: { errors, isSubmitting }, reset } = useForm<FormProps>({
    mode: "onSubmit",
    ...
    <div className="flex flex-col items-center justify-center min-h-screen">
      <div className="rounded hover:shadow-xl bg-white p-7 mt-4 max-w-sm border-y-4 border-teal-500">
        ...
        <NavLink to='/login' className="text-right">
          <p className="text-slate-500 hover:text-slate-700 text-sm">Já possuí cadastro? Logar</p>
        </NavLink>
      </div>
